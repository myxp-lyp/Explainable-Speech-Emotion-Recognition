...
def to_waf_dataset(batch, xcs, xds, label_fn, combo_features=None,
    ...
    y_trues = torch.tensor([label2id_map[item] for item in batch["emotion"]])
    y_trues = F.one_hot(y_trues, num_classes=len(label2id_map)).float()
...
def create_waf_dataset(
    ...
    waf_dataset = waf_dataset.map(
        to_waf_dataset,
        fn_kwargs={"xcs": config.xcs, "xds": config.xds,
                   "label_fn": bce_per_class,
