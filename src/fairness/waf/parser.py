...
def parse_waf_metrics(waf_model: WAFModel, config: WAFConfig):
    ...
    # dataset.py concatenates features in order: [xcs, xds] (line 32-37).
    # Keep the same order here so demographic_columns index into linear weights
    # correctly.
    columns = config.xcs + config.xds
